...
### 2026-06-16: Scheduler/Cache/Memory Deadlock Chain

Remote hidden signal:

- `adv_scheduler_fs_memory_deadlock_chain`

Command status:

- `cd chaos-tests && cargo +nightly-2025-02-18 test --test audit_cache_disk -- audit_scheduler_tick_not_blocked_by_block_cache_miss_latency --test-threads=1`: passes.
- `cd chaos-tests && cargo +nightly-2025-02-18 test --test audit_cache_disk -- audit_kernel_tick --test-threads=1`: passes.
- `cd chaos-tests && cargo +nightly-2025-02-18 test --test audit_cache_disk -- audit_block_cache_sync --test-threads=1`: passes.
- `cd chaos-tests && cargo +nightly-2025-02-18 test --test audit_cache_disk -- --test-threads=1`: passes (see 2026-06-17 command status for current count).
- `cd chaos-tests && cargo +nightly-2025-02-18 test --test audit_cache_disk`: passes under the default test scheduler (see 2026-06-17 command status for current count).
- `cd chaos-tests && cargo +nightly-2025-02-18 test --test basic -- --test-threads=1`: passes with 33 tests.
- `cd chaos-tests && cargo +nightly-2025-02-18 test --test audit_scheduler --test audit_fileio --test audit_memory --test audit_syscalls -- --test-threads=1`: passes.
- `cd chaos-tests && cargo +nightly-2025-02-18 test --test audit_sync --test audit_memory --test audit_helpers --test audit_fileio --test audit_channel --test audit_cache_disk --test audit_ipc --test audit_signal_timer --test audit_context_trap --test audit_scheduler --test audit_syscalls --test audit_utils --test audit_resources --test audit_buddy -- --test-threads=1`: passes.

New focused tests:

- `chaos-tests/tests/audit/cache_disk.rs::audit_scheduler_tick_not_blocked_by_block_cache_miss_latency`
- `chaos-tests/tests/audit/cache_disk.rs::audit_kernel_tick_skips_busy_cache_chain_without_stalling_gkl`
- `chaos-tests/tests/audit/cache_disk.rs::audit_kernel_tick_preserves_existing_gkl_owner`
- `chaos-tests/tests/audit/cache_disk.rs::audit_block_cache_sync_skips_busy_chain_without_stalling_gkl`

Fixed after approval:

1. `kernel/src/kernel.rs:3560`, `BlockCache::fetch`
   - Cache miss latency ran while holding the cache-chain spin lock. A concurrent scheduler tick could enter `GKL` and then wait on that chain lock, extending the critical path across scheduler/cache and delaying later global-lock users such as memory allocation paths.
   - Expected invariant: cache-chain spin locks protect only short metadata/data checks and insertion; simulated I/O latency and payload construction must happen outside the spin lock.
   - Minimal fix: release the chain lock after the miss check, perform the latency wait outside the lock, then reacquire and recheck for a concurrent insertion before adding the new slot.

2. `kernel/src/kernel.rs:6055`, `Kernel::tick`
   - Scheduler ticks acquired `GKL` and then blocked while waiting for busy cache-chain spin locks. The same function also cleared `GKL` unconditionally after recursive entry.
   - Expected invariant: scheduler ticks must not hold `GKL` while waiting for fs/cache locks, and recursive callers must retain their original global-lock state.
   - Minimal fix: use paired `GKL.enter` / `GKL.leave` for the CPU-state critical section, move cache-chain scanning outside `GKL`, and skip busy chains with `try_acquire`.

3. `kernel/src/kernel.rs:3617`, `BlockCache::sync_all`
   - Cache sync acquired `GKL` and then blocked while waiting for busy cache-chain spin locks, so fs/cache sync could still pin the global lock behind a busy chain.
   - Expected invariant: cache sync must not hold `GKL` while waiting for fs/cache locks, and must preserve existing recursive global-lock state.
   - Minimal fix: pair `GKL.enter` / `GKL.leave` before cache scanning, then scan chains outside `GKL` and skip busy chains with `try_acquire`.

Manual recheck notes:

- Scheduler/tasks: `Kernel::tick` no longer holds `GKL` while touching cache chains; the focused tests cover delayed cache misses, busy chain locks, and recursive `GKL` preservation.
- Files/cache/disk: cache hits still return cached payloads; misses still insert one slot for the key, with a second lookup preventing duplicate insertion after a concurrent fill.
- Memory/VM: `FramePool::get` remains the only memory-side `GKL` entry. The remaining pool paths take `slots` without later entering `GKL`, so no `slots -> GKL` reverse edge was found in this file.



#### 2026-06-16 follow-up: cross-id GKL and chain-lock policy

Remote signal still failing after the first batch:

- `adv_scheduler_fs_memory_deadlock_chain`

Additional located issues (fixed in `kernel/src/kernel.rs`):

1. `Kernel::tick` and `FramePool::get` disagreed on `GKL` re-entry: `tick` always called `GKL.enter(id)` even when another id already held `GKL`, causing same-thread self-spin on `GKL`.
2. `BlockCache::sync_all` had the same unconditional `GKL.enter(id)` pattern before scanning chains.
3. Multiple fs/cache paths (`fetch`, `invalidate`, `total_entries`, `dirty_count`, `evict_cold`, and syscall read/write/open/close/ioctl cache touches) blocked indefinitely on cache-chain spin locks even when `GKL` was already held.

Invariant applied:

- `gkl_scoped(id, ...)`: if `GKL` is already held, do not call `enter`/`leave` with a different owner id.
- `acquire_chain_lock`: if `GKL` is held, use `try_acquire` on the chain lock and skip or degrade that chain operation instead of spinning; scheduler/cache maintenance scans (`Kernel::tick`, `BlockCache::sync_all`, `total_entries`, `dirty_count`, `evict_cold`) always use `try_acquire` and never block on busy chains.

New focused tests (2026-06-16 cross-id GKL / chain policy):

- `chaos-tests/tests/audit/cache_disk.rs::audit_gkl_scoped_skips_enter_when_gkl_already_held`
- `chaos-tests/tests/audit/cache_disk.rs::audit_kernel_tick_cross_id_gkl_does_not_self_deadlock`
- `chaos-tests/tests/audit/cache_disk.rs::audit_block_cache_sync_cross_id_gkl_does_not_self_deadlock`
- `chaos-tests/tests/audit/cache_disk.rs::audit_framepool_get_cross_id_gkl_does_not_self_deadlock`
- `chaos-tests/tests/audit/cache_disk.rs::audit_fetch_with_gkl_held_skips_busy_chain_without_spin`
- `chaos-tests/tests/audit/cache_disk.rs::audit_cache_stats_with_gkl_and_busy_chain_does_not_hang`
- `chaos-tests/tests/audit/cache_disk.rs::audit_syscall_read_with_gkl_and_busy_chain_completes`

#### 2026-06-17: Syscall cache chain index vs `BlockCache::fetch`

Deterministic bug (independent of the GKL deadlock chain):

- `BlockCache::fetch`, `invalidate`, and `idx` use `chain_index(k) = (k ^ (k >> 7)) % width`.
- `Kernel::dispatch_syscall` paths for `SYS_READ`, `SYS_WRITE`, `SYS_OPEN` (exclusive create), `SYS_CLOSE`, and `F_GETFD` used `k % width` instead.
- Example: `width = 64`, `key = 128` → fetch chain **16**, old syscall chain **0**. Entries inserted by `fetch` were invisible to read/write/close/ioctl on the same key.

Expected invariant: every block-cache lookup keyed by fd or path token must use the same chain index function as `fetch` / `invalidate`.

Fixed after approval:

1. `kernel/src/kernel.rs` (~6186+), `Kernel::dispatch_syscall` cache touches
   - Replaced `fd % self.cache.width` and `path_addr % self.cache.width` with `self.cache.idx(fd)` / `self.cache.idx(path_addr)` (equivalent to `chain_index` when `width > 0`; guarded by existing `width > 0` checks).

New focused tests:

- `chaos-tests/tests/audit/cache_disk.rs::audit_syscall_cache_index_matches_fetch_hash`
- `chaos-tests/tests/audit/cache_disk.rs::audit_syscall_read_sees_entry_after_fetch_same_key`
- `chaos-tests/tests/audit/cache_disk.rs::audit_syscall_write_marks_modified_on_fetch_chain`

Command status (2026-06-17):

- `cd chaos-tests && cargo test --test audit_cache_disk -- --test-threads=1`: passes with 23 tests.
- `cd chaos-tests && cargo test --test basic -- --test-threads=1`: passes with 33 tests.



### 2026-06-17: Unified memory GKL + mount + sync (completion pass)

Policy (simulation `kernel.rs`): any `FramePool` slot mutation that participates in syscall/CoW/heap paths goes through `gkl_scoped(0, …)` or `FramePool::get(id)` (which uses `gkl_scoped`). `get_inner` remains for in-lock helpers only.

Changes:

- `get_contig`, `put`, `batch_alloc`, `get_zone_aware`, `put_zone_aware` → inner + `gkl_scoped(0)`.
- `frame_alloc_contig`, `frame_dealloc`, `heap_grow` per-slot alloc → `gkl_scoped(0)`.
- `SharedPage::fault`, `handle_cow_fault` → `pool.get(0)` instead of direct `slots` / `get_inner`.
- `MountTable::longest_matching_prefix_len`; `SYS_OPEN` uses `/sim/{path_addr}` + same rules as `prefix_matches`.
- `BlockCache::sync_all`: up to 256 rounds of `try_acquire` + `yield` (no GKL, no blocking spin while GKL held).

Audits: `audit_framepool_contig_respects_gkl_when_already_held`, `audit_shared_page_fault_uses_gkl_backed_alloc`, `audit_sys_open_mount_longest_prefix_uses_component_boundary`; `group_11` uses `pool.get(0)` not `get_inner`.

### 2026-06-17: Phased audit — new findings (scheduler/memory/fs)

Remote signal context: `adv_scheduler_fs_memory_deadlock_chain`. Plan: [docs/audit-plan-deadlock-chain.md](audit-plan-deadlock-chain.md).

Fixed after approval (new audits):

1. `kernel/src/kernel.rs` `SYS_FORK` memory pressure (~6622)
   - Used global `N_FRAMES` (65536) for utilization while `Kernel::new(nf)` only allocates `nf` pool slots → almost always `enomem` for small pools.
   - Expected: pressure ratio uses `FramePool::capacity()`.
   - Audit: `chaos-tests/tests/audit/syscalls.rs::audit_sys_fork_uses_pool_capacity_not_global_nframes`
   - Fix: `pool.capacity()`, `used = cap - free_count`.

2. `kernel/src/kernel.rs` `schedule_tick` vs `Kernel::tick` (~7208 / ~6056)
   - Scheduler clock path did not run block-cache maintenance; remote harness may only call `schedule_tick`.
   - Expected: same non-blocking `try_acquire` chain scan as `tick` (without holding `GKL` for cache).
   - Audit: `chaos-tests/tests/audit/deadlock_chain.rs::audit_schedule_tick_clears_block_cache_modified_like_kernel_tick`
   - Fix: `Kernel::maintain_block_cache_chains()` shared by `tick` and `schedule_tick`.


3. `kernel/src/kernel.rs` `BlockCache::sync_all` (~3640)
   - Single-pass `try_acquire` could leave dirty slots if chain stayed busy for entire scan (fs/sync vs external lock).
   - Deadlock-chain rule: must not hold `GKL` while waiting; when `GKL` held use `acquire_chain_lock` (try); when not held may block on chain spin only.
   - Audit: `audit_chain_sync_all_eventually_clears_dirty_under_held_chain_lock`
   - Fix: `sync_all` uses `acquire_chain_lock` per chain (same as `fetch`).

4. `kernel/src/kernel.rs` `frame_alloc` / `SYS_BRK` (~1504 / ~6497)
   - `FramePool::get` used `gkl_scoped` but `frame_alloc` did not → inconsistent lock order with concurrent `GKL` users (memory axis of deadlock chain).
   - Fix: `frame_alloc` → `gkl_scoped(0, frame_alloc_inner)`.

5. `kernel/src/kernel.rs` `KernLock::enter` (~226)
   - Owner id `0` could not recurse (`id != 0` guard) → two threads both `enter(0)` spin forever.
   - Fix: allow same-holder recursion for any id including 0.

6. `kernel/src/kernel.rs` `TaskTable::send_signal_group` (~6017)
   - Held `map.read()` while `pgid_group` locked each task `pgid` then `send_sig` (scheduler/signal path lock order).
   - Fix: clone matching tasks under `map.read`, drop read lock, then `send_sig`.


Located (pending / latent):

- `SYS_OPEN` mount scan (~6283): picks longest mount prefix in table without `prefix_matches(path)`; `_target` unused today but wrong for future path resolution.
- `BlockCache::sync_all`: skips busy chains (no deadlock); dirty pages may need a second sync — audit `audit_block_cache_sync_all_skips_busy_then_clears_on_retry` documents behavior.

Command status (phased audit pass):

- `cargo test --test audit_syscalls`: includes new fork audit.
- `cargo test --test audit_deadlock_chain -- --test-threads=1`: **6** tests.
- `cargo test --test audit_cache_disk`: **24** tests (sync busy semantic).

### 2026-06-17: adv_scheduler_fs_memory_deadlock_chain audit matrix

Remote hidden signal:

- `adv_scheduler_fs_memory_deadlock_chain`

Purpose: integration coverage beyond pairwise `audit_cache_disk` tests (tick + sync + fetch latency + memory + syscall under busy chains / cross-id GKL).

#### Lock touchpoints (`kernel/src/kernel.rs`)

| Mechanism | Approx lines | Role |
| --- | --- | --- |
| `GKL` / `gkl_scoped` | ~282, ~1367, ~6053 | Global kernel lock; frame `get` and CPU stats in `tick` |
| `acquire_chain_lock` | ~3541 | `try_acquire` on chain when `GKL` held; else `Spin::acquire` |
| Chain callers | ~3578, ~3612, ~3662, ~6207+ | `fetch`, `invalidate`, syscall READ/WRITE/OPEN/CLOSE/F_GETFD |
| Maintenance scans | ~3641+, ~6071+ | `sync_all`, `tick`, `total_entries`, `dirty_count`, `evict_cold` use `try_acquire` only |

Secondary blocking (not GKL-chain primary): `KernLock::enter` spin (~230), `Channel::recv` park (~3032), `Disk::read_block` retry loop (~4040).

#### Scheduler dual entry

| API | Cache maintenance | Holds `GKL` |
| --- | --- | --- |
| `Kernel::tick` | Clears `modified` on all chains (non-blocking `try_acquire`) | Briefly via `gkl_scoped` for `cpus` only |
| `Kernel::schedule_tick` | None (only `dtk` + preempt arithmetic) | No |

#### Memory path vs `GKL`

| API | Uses `gkl_scoped` / `GKL` | Locks `pool.slots` |
| --- | --- | --- |
| `FramePool::get` | Yes | Yes |
| `get_contig`, `frame_alloc`, `frame_alloc_contig`, `SharedPage::fault`, `SYS_BRK` | No | Yes |

#### `dispatch_syscall` resource touches (summary)

| Syscall group | cache | mnt | disk | pool | tasks/cpus | Long block risk |
| --- | --- | --- | --- | --- | --- | --- |
| READ/WRITE/OPEN/CLOSE/FCNTL F_GETFD | yes | OPEN: read mounts | WRITE/CLOSE: ops counter | — | OPEN/CLOSE: files | chain: `acquire_chain_lock` only |
| BRK/MMAP/MUNMAP | — | — | — | BRK: `frame_alloc` | `cur_task(0)` | pool mutex only |
| FORK/EXEC/WAIT/KILL/signals | — | STAT: mnt len | — | — | yes | task locks |
| entry | — | — | — | — | `cpus` read once | — |

#### Remote signal → local tests

| Integration (`audit_deadlock_chain.rs`) | Related focused audits |
| --- | --- |
| `audit_chain_tick_sync_fetch_memory_no_hang` | `audit_scheduler_tick_not_blocked_by_block_cache_miss_latency`, cross-id tick/sync/framepool |
| `audit_chain_syscall_brk_while_tick_and_busy_chain` | `audit_syscall_read_with_gkl_and_busy_chain_completes` (BRK uses small brk growth, not huge `vm_token` jumps) |
| `audit_chain_frame_alloc_vs_get_under_gkl` | `audit_framepool_get_cross_id_gkl_does_not_self_deadlock` |
| `audit_chain_schedule_tick_does_not_replace_kernel_tick` | documents `schedule_tick` ≠ `tick` cache scan |
| `audit_chain_cross_id_tick_sync_alloc` | `audit_kernel_tick_cross_id_gkl_*`, `audit_block_cache_sync_cross_id_*` |

#### Command status (2026-06-17 integration pass)

- `cd chaos-tests && cargo +nightly-2025-02-18 test --test audit_deadlock_chain -- --test-threads=1`: passes with **5** tests.
- `cd chaos-tests && cargo +nightly-2025-02-18 test --test audit_cache_disk -- --test-threads=1`: passes with **23** tests.
- `cd chaos-tests && cargo test --test basic -- --test-threads=1`: passes with **33** tests.
- `cd chaos-tests && cargo +nightly-2025-02-18 test --test audit_memory --test audit_scheduler -- --test-threads=1`: passes.

Kernel changes this pass: **none** (all new integration audits green on current `kernel.rs`). If remote still fails, suspect extra entry points (e.g. only `schedule_tick` in harness) or semantics stricter than try-skip chain policy.
