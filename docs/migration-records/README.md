...
| [batches/B03-fs.md](batches/B03-fs.md) | B03 fs 的 file、mmap、epoll、pipe、TTY/channel 详细迁移记录和源码修改前报告材料；append/seek、mmap range、epoll state、pipe endpoint、TTY/channel wakeup 子项已完成。 |
| [batches/B04-ipc.md](batches/B04-ipc.md) | B04 IPC 的 semaphore、undo/remove、shared memory key/size 详细迁移记录和源码修改前报告材料；三个子项均已完成。 |
| [batches/B05-process.md](batches/B05-process.md) | B05 process 的 stack、process brk、fd lifecycle/cloexec、fork/wait、ELF bounds 详细迁移记录和源码修改前报告材料；核心子项均已完成。 |
...
4. `kernel/src/fs/`，其中 append/seek、mmap range、epoll state、pipe endpoint、TTY/channel wakeup 子项已完成；fd/open-file lifecycle 联动项已由 B05.3 收口。
5. `kernel/src/ipc/` 已完成 B04 三个子项。
6. `kernel/src/process/` 和 syscall process/fd 生命周期；其中 stack、process brk、fd lifecycle/cloexec、ELF bounds 与 fork/wait parent-child 核心子项已完成。
7. `kernel/src/signal/*` 和架构 trap/context 后续按真实运行边界进入，不作为当前已恢复新增路径的独立批次。
