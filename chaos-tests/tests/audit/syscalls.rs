...
#[test]
fn audit_sys_fork_uses_pool_capacity_not_global_nframes() {
    let (kernel, _task) = kernel_with_root_frames(64);
    let before = kernel.pool.free_count();
    assert!(before > 0);
    let fork = kernel.dispatch_syscall(SYS_FORK, 0, 0, 0, 0, 0, 0);
    assert!(
        fork.is_ok(),
        "fork must not spuriously fail enomem when pool still has free frames (had {before})"
    );
}

#[test]
fn audit_sys_open_mount_longest_prefix_uses_component_boundary() {
    let (kernel, _root) = kernel_with_root();
    kernel.mnt.bind("/mnt", "dev0");
    let path_addr = 0x2000usize;
    let path_sim = format!("/sim/{:x}", path_addr);
    assert_eq!(kernel.mnt.longest_matching_prefix_len(&path_sim), 0);
    kernel.mnt.bind("/sim/2000", "dev1");
    assert_eq!(
        kernel.mnt.longest_matching_prefix_len(&path_sim),
        "/sim/2000".len()
    );
    assert_eq!(
        kernel.mnt.longest_matching_prefix_len("/mnted/file"),
        0,
        "/mnt must not match /mnted"
    );
}
