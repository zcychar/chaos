...
impl MemoryArea {
    ...
    fn check_read_array<S>(&self, ptr: *const S, count: usize) -> usize {
        ...
        let bytes = match size_of::<S>().checked_mul(count) {
            Some(bytes) => bytes,
            None => return 0,
        };
        // page align
        let min_bound = ptr.max(Page::of_addr(self.start_addr).start_address());
        let ptr_end = match ptr.checked_add(bytes) {
            Some(end) => end,
            None => return 0,
        };
        let area_end = match Self::align_up(self.end_addr) {
            Some(end) => end,
            None => return 0,
        };
        let max_bound = ptr_end.min(Page::of_addr(area_end).start_address());
    ...
    pub fn is_overlap_with(&self, start_addr: VirtAddr, end_addr: VirtAddr) -> bool {
        if self.start_addr >= self.end_addr || start_addr >= end_addr {
            return false;
        }
...
impl<T: PageTableExt> MemorySet<T> {
    ...
    pub fn find_free_area(&self, addr_hint: usize, len: usize) -> VirtAddr {
        assert!(len > 0, "invalid memory area length");
        // brute force:
        // try each area's end address as the start
        core::iter::once(addr_hint)
            .chain(self.areas.iter().map(|area| area.end_addr))
            .filter_map(Self::align_up)
            .filter_map(|addr| addr.checked_add(len).map(|end| (addr, end)))
            .find(|&(addr, end)| self.test_free_area(addr, end))
            .map(|(addr, _)| addr)
    ...
    pub fn push(
        ...
        end_addr = match Self::align_up(end_addr) {
            Some(end_addr) => end_addr,
            None => return,
        };
        if start_addr >= end_addr || !self.test_free_area(start_addr, end_addr) {
            return;
        }
        let area = MemoryArea {
            start_addr,
    ...
    pub unsafe fn with<R>(&self, f: impl FnOnce() -> R) -> R {
