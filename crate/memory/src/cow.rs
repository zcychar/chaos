...
impl FrameRcMap {
    ...
    fn read_increase(&mut self, frame: &Frame) {
        ...
        self.map().insert(frame.clone(), (r.saturating_add(1), w));
    }
    /*
     ...
     **  @retval none
     */
    fn read_decrease(&mut self, frame: &Frame) -> bool {
        self.decrease(frame, true)
    }
    /*
    ...
    fn write_increase(&mut self, frame: &Frame) {
        let (r, w) = self.map().get(&frame).unwrap_or(&(0, 0)).clone();
        self.map().insert(frame.clone(), (r, w.saturating_add(1)));
    }
    /*
     ...
     **  @retval none
     */
    fn write_decrease(&mut self, frame: &Frame) -> bool {
        self.decrease(frame, false)
    }

    fn decrease(&mut self, frame: &Frame, read: bool) -> bool {
        let map = self.map();
        let remove = match map.get_mut(frame) {
            Some((r, w)) => {
                if read {
                    if *r == 0 {
                        return false;
