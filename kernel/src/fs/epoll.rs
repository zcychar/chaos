...
pub struct EpollInstance {
    pub events: Arc<SpinNoIrqLock<BTreeMap<usize, EpollEvent>>>,
    pub ready_list: Arc<SpinNoIrqLock<BTreeSet<usize>>>,
    pub new_ctl_list: Arc<SpinNoIrqLock<BTreeSet<usize>>>,
}
...
impl Clone for EpollInstance {
    fn clone(&self) -> Self {
        EpollInstance {
            ...
            new_ctl_list: self.new_ctl_list.clone(),
        }
    }
}
...
impl EpollInstance {
    pub fn new(_flags: usize) -> Self {
        return EpollInstance {
            events: Arc::new(SpinNoIrqLock::new(BTreeMap::new())),
            ready_list: Arc::new(SpinNoIrqLock::new(BTreeSet::new())),
            new_ctl_list: Arc::new(SpinNoIrqLock::new(BTreeSet::new())),
        };
    }

    pub fn control(&mut self, op: usize, fd: usize, event: Option<&EpollEvent>) -> SysResult {
        match op as i32 {
            EPollCtlOp::ADD => {
                let event = event.ok_or(SysError::EFAULT)?;
                let mut events = self.events.lock();
                if events.contains_key(&fd) {
                    return Err(SysError::EEXIST);
                }
                events.insert(fd, event.clone());
                drop(events);
            ...
            EPollCtlOp::DEL => {
                let mut events = self.events.lock();
                if events.remove(&fd).is_some() {
                    drop(events);
                    self.ready_list.lock().remove(&fd);
                    self.new_ctl_list.lock().remove(&fd);
                } else {
                    return Err(SysError::EPERM);
                }
            }
...
impl EPollCtlOp {
    pub const ADD: i32 = 1; /* Add a file descriptor to the interface.  */
    pub const DEL: i32 = 2; /* Remove a file descriptor from the interface.  */
    pub const MOD: i32 = 3; /* Change file descriptor epoll_event structure.  */
}
