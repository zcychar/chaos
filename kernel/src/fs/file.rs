...
impl FileHandle {
    ...
    pub fn is_broken_pipe_write(&self) -> bool {
        self.pipe
            && self
                .inode
                .as_any_ref()
                .downcast_ref::<crate::fs::Pipe>()
                .map_or(false, |pipe| pipe.is_write_closed())
    }
    ...
    pub fn write(&mut self, buf: &[u8]) -> Result<usize> {
        ...
        let offset = match description.options.append {
            true => self.inode.metadata()?.size,
            false => {
                if description.offset > usize::MAX as u64 {
                    return Err(FsError::InvalidParam);
                }
                description.offset as usize
            }
        };
        drop(description);
        ...
        self.description.write().offset = new_offset;
        Ok(len)
    }
    ...
    pub fn write_at(&self, offset: usize, buf: &[u8]) -> Result<usize> {
        ...
        offset.checked_add(buf.len()).ok_or(FsError::InvalidParam)?;
    ...
    pub fn seek(&mut self, pos: SeekFrom) -> Result<u64> {
        ...
        let next = match pos {
            ...
            SeekFrom::End(offset) => self.inode.metadata()?.size as i128 + offset as i128,
            SeekFrom::Current(offset) => description.offset as i128 + offset as i128,
        };
    ...
    pub fn mmap(&mut self, area: MMapArea) -> Result<()> {
        ...
        let mapping_len = area
            .end_vaddr
            .checked_sub(area.start_vaddr)
            .filter(|len| *len != 0)
            .ok_or(FsError::InvalidParam)?;
