...
impl Thread {
    ...
    pub fn new_user_vm(
        ...
        // Make page table
        let bias = elf.make_memory_set(&mut new_vm, inode)?;
        ...
        if let Ok(loader_path) = elf.get_interpreter() {
            ...
            elf_interp.append_as_interpreter(&interp_inode, &mut new_vm, bias)?;
        ...
        let mut ustack_top = {
            ...
            let ustack_top = USER_STACK_OFFSET
                .checked_add(USER_STACK_SIZE)
                .ok_or("user stack range overflow")?;
            let eager_stack_size = PAGE_SIZE
                .checked_mul(4)
                .ok_or("user stack eager size overflow")?;
            let eager_stack_start = ustack_top
                .checked_sub(eager_stack_size)
                .ok_or("user stack eager range underflow")?;
        ...
        ustack_top = init_info.try_push_at_in_vm(&mut new_vm, ustack_top)?;
    ...
    pub fn new_user(
        ...
        let thread = Thread {
            ...
            proc: Arc::new(Mutex::new(Process {
                vm,
                files,
                fd_cloexec: BTreeSet::new(),
                cwd: String::from("/"),
                exec_path: String::from(exec_path),
                futexes: BTreeMap::default(),
                semaphores: SemProc::default(),
                brk_start: crate::process::USER_BRK_START,
                brk: crate::process::USER_BRK_START,
    ...
    pub fn fork(&self, tf: &UserContext) -> Arc<Thread> {
        ...
        let new_proc = Arc::new(Mutex::new(Process {
            ...
            fd_cloexec: proc.fd_cloexec.clone(),
            cwd: proc.cwd.clone(),
