...
static ACTIVE_MEMORY_SET: SpinNoIrqLock<Option<usize>> = SpinNoIrqLock::new(None);
...
impl FrameAllocator for GlobalFrameAlloc {
    ...
    fn alloc_contiguous(&self, size: usize, align_log2: usize) -> Option<PhysAddr> {
        if size == 0 || align_log2 >= mem::size_of::<usize>() * 8 {
            return None;
        }
        // get the real address of the alloc frame
        let ret = FRAME_ALLOCATOR
            ...
            .and_then(|id| id.checked_mul(PAGE_SIZE))
            .and_then(|addr| addr.checked_add(MEMORY_OFFSET));
        trace!("Allocate frame: {:x?}", ret);
        ret
        // TODO: try to swap out when alloc failed
    }
    fn dealloc(&self, target: usize) {
        ...
        if let Some(offset) = target.checked_sub(MEMORY_OFFSET) {
            FRAME_ALLOCATOR.lock().dealloc(offset / PAGE_SIZE);
        }
    }
}
...
struct ActiveMemorySetGuard {
    previous: Option<usize>,
}

impl Drop for ActiveMemorySetGuard {
    fn drop(&mut self) {
        *ACTIVE_MEMORY_SET.lock() = self.previous;
    }
}

pub unsafe fn with_active_memory_set<T>(vm: &mut MemorySet, f: impl FnOnce() -> T) -> T {
    let previous = {
        let mut active = ACTIVE_MEMORY_SET.lock();
        let previous = *active;
        *active = Some(vm as *mut MemorySet as usize);
        previous
    };
    let _guard = ActiveMemorySetGuard { previous };
    #[cfg(any(target_arch = "riscv32", target_arch = "riscv64"))]
    {
        let sum_enabled = riscv::register::sstatus::read().sum();
...
pub fn handle_page_fault(addr: usize) -> bool {
    ...
    if let Some(thread) = current_thread() {
        ...
        return lock.handle_page_fault(addr);
    }
    let active = *ACTIVE_MEMORY_SET.lock();
    if let Some(ptr) = active {
        return unsafe { (&mut *(ptr as *mut MemorySet)).handle_page_fault(addr) };
    }
    false
}
...
pub fn handle_page_fault_ext(addr: usize, access: crate::memory::AccessType) -> bool {
    ...
    if let Some(thread) = current_thread() {
        let mut lock = thread.vm.lock();
        return lock.handle_page_fault_ext(addr, access);
    }
    let active = *ACTIVE_MEMORY_SET.lock();
    if let Some(ptr) = active {
        return unsafe { (&mut *(ptr as *mut MemorySet)).handle_page_fault_ext(addr, access) };
    }
    false
}
...
pub fn access_ok(addr: usize, len: usize) -> bool {
    addr <= PHYSICAL_MEMORY_OFFSET
        && addr
            .checked_add(len)
            .map_or(false, |end| end <= PHYSICAL_MEMORY_OFFSET)
}
